...
import {
  ...
  RotateCcw, Hand, Sparkle, Search, CircleCheck
} from 'lucide-react'
        ...
        {!selectionMode && (
          <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-all flex items-center justify-center opacity-0 group-hover:opacity-100">
            <button
              ...
              className="p-2 bg-red-500/80 rounded-full ios-press"
            >
        ...
        <div className="p-4">
          ...
          <div className="flex gap-2 mt-3">
            {photo.category.filter(c => c !== 'allPhotos').map(c => (
              <span key={c} className="text-xs bg-white/10 text-white/70 px-2 py-1 rounded-full capitalize">
                {c}
              </span>
            ))}
          </div>
...
export default function CleanupApp() {
  ...
  const totalDeviceSize = 128 * 1073741824 // 128 GB simulated
  ...
  const handleImport = useCallback(async (files: FileList | null) => {
    ...
    for (let i = 0; i < Math.min(files.length, store.maxPhotos - store.photos.length); i++) {
      const file = files[i]
      const url = URL.createObjectURL(file)
      const categories = ['allPhotos']

      if (file.type.startsWith('video/')) categories.push('videos')
      if (file.name.toLowerCase().includes('screenshot')) categories.push('screenshots')
      if (file.name.toLowerCase().includes('selfie')) categories.push('selfies')
      ...
      newPhotos.push({
        ...
        isScreenshot: file.name.toLowerCase().includes('screenshot'),
        isSelfie: file.name.toLowerCase().includes('selfie'),
      })
    }
      ...
      <div className="min-h-screen bg-black flex flex-col items-center justify-center px-6">
        ...
        >
          <svg viewBox="0 0 100 100" className="w-full h-full">
            ...
            <circle
              cx="50" cy="50" r="40" fill="none" stroke="#30d158" strokeWidth="6"
              strokeLinecap="round"
              strokeDasharray={`${scanProgress * 2.51} 251`}
              className="transition-all duration-200"
            />
          </svg>
        </motion.div>
        <h2 className="text-white text-2xl font-bold mb-2">Scanning Photos...</h2>
        <p className="text-[#8e8e93] text-center mb-4">
          Finding duplicates, blurry shots, and clutter
        </p>
      ...
      <div className="min-h-screen bg-black flex flex-col items-center justify-center px-6">
        ...
        >
          ...
          <motion.button
            ...
            className="w-full max-w-xs mx-auto bg-[#1c1c1e] text-[#30d158] font-semibold text-base py-3.5 rounded-2xl border border-[#30d158]/20 ios-press"
          >
            <div className="flex items-center justify-center gap-2">
    ...
    return (
      <div className="min-h-screen bg-black flex flex-col">
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-6">
          <div
            className="w-full max-w-sm border-2 border-dashed border-[#38383a] rounded-3xl p-10 text-center hover:border-[#30d158] transition-colors cursor-pointer"
          ...
          >
            <ArrowDownToLine className="w-12 h-12 text-[#8e8e93] mx-auto mb-4" />
            <h3 className="text-white text-xl font-semibold mb-2">Import Your Photos</h3>
            <p className="text-[#8e8e93] text-sm mb-6">
              Drag and drop photos here, or tap to browse
            </p>
            <div className="inline-flex items-center gap-2 bg-[#30d158] text-white font-semibold px-6 py-3 rounded-xl ios-press">
      ...
      <div className="min-h-screen bg-black flex flex-col items-center justify-center px-6">
        ...
        >
          ...
          <div className="bg-[#1c1c1e] rounded-2xl p-6 w-full max-w-xs mx-auto mb-8">
            <div className="text-[#30d158] text-3xl font-bold mb-1">
    ...
    return (
      <div className="min-h-screen bg-black flex flex-col">
            ...
            >
              <div className="flex items-center justify-between px-4 py-3">
                <span className="text-[#30d158] font-medium">
                  {store.selectedPhotos.size} selected · {formatSize(selectedSize)}
                </span>
                <div className="flex gap-3">
                  <button
                    onClick={() => {
                      const allIds = categoryPhotos.map(p => p.id)
                      store.selectAll(allIds)
                    }}
                    className="text-[#0a84ff] text-sm font-medium ios-press"
                  >
                    Select All
                  </button>
                  <button
                    onClick={handleDelete}
                    className="text-[#ff453a] text-sm font-semibold ios-press"
                  >
                    Delete
                  </button>
                </div>
              ...
              <AnimatePresence mode="popLayout">
                {categoryPhotos.map((photo) => (
                  <PhotoCard
                    key={photo.id}
                    photo={photo}
                    isSelected={store.selectedPhotos.has(photo.id)}
                    onToggle={() => store.toggleSelect(photo.id)}
                    onDelete={() => {
          ...
          <div className="fixed bottom-0 left-0 right-0 ios-backdrop border-t border-[#38383a] safe-bottom">
            <div className="flex items-center justify-around py-3 px-4">
              ...
              >
                ...
                <span className="text-[#8e8e93] text-[10px]">Cancel</span>
              </button>
              <button
                onClick={handleDelete}
                className="flex flex-col items-center gap-1 ios-press"
              >
                ...
                <span className="text-[#ff453a] text-[10px] font-medium">
                  Delete ({store.selectedPhotos.size})
                </span>
              </button>
              <button
                onClick={() => {
              ...
              >
                ...
                <span className="text-[#30d158] text-[10px]">Select All</span>
              </button>
            </div>
          </div>
        )}
        ...
        <AnimatePresence>
          {showDeleteConfirm && (
            <DeleteConfirmDialog
              count={store.selectedPhotos.size}
        ...
        <div className="flex items-center px-4 py-3 border-b border-[#38383a]">
          ...
          <div className="flex-1 text-center">
            ...
            <p className="text-[#8e8e93] text-xs">
              {swipeIndex} of {swipePhotos.length} reviewed
            </p>
          </div>
          <div className="w-10" />
        </div>
          ...
          {!currentPhoto ? (
            <div className="flex flex-col items-center justify-center h-full">
              ...
              <p className="text-[#8e8e93] text-center mb-6">
                You&apos;ve reviewed all photos in this category
              </p>
              <button
                onClick={() => {
                  store.setScreen('home')
                ...
                className="bg-[#30d158] text-white font-semibold px-6 py-3 rounded-xl ios-press"
              >
                Back to Home
              </button>
            </div>
  ...
  if (store.currentScreen === 'smartClean') {
    const cleanablePhotos = categoryStats
      .filter(c => c.id !== 'allPhotos' && c.count > 0)
      ...
      <div className="min-h-screen bg-black flex flex-col">
        ...
        <div className="fixed bottom-0 left-0 right-0 p-4 ios-backdrop safe-bottom">
          <motion.button
            whileTap={{ scale: 0.97 }}
            onClick={() => {
              const allIds = cleanablePhotos.flatMap(cat =>
                store.getPhotosByCategory(cat.id).map(p => p.id)
              )
              store.selectAll(allIds)
              setShowDeleteConfirm(true)
            }}
    ...
    <div className="min-h-screen bg-black flex flex-col">
      ...
      <div className="px-4 pt-12 pb-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex gap-2">
            <motion.button
              whileTap={{ scale: 0.9 }}
              onClick={() => store.setScreen('import')}
        ...
        <p className="text-[#8e8e93] text-sm">
          {store.photos.length} photos · {formatSize(totalSize)} used
        </p>
      </div>
      ...
      <div className="px-4 mb-6">
        <div className="grid grid-cols-2 gap-3">
          <motion.button
            whileTap={{ scale: 0.97 }}
            onClick={() => {
              store.setActiveCategory('duplicates')
              store.setScreen('swipe')
              setSwipeIndex(0)
            }}
          ...
          <motion.button
            whileTap={{ scale: 0.97 }}
            onClick={() => store.setScreen('smartClean')}
            className="bg-[#1c1c1e] rounded-2xl p-4 text-left ios-press"
          >
            ...
            <p className="text-[#8e8e93] text-xs mt-0.5">
              {activeCategories.length > 0
                ? `${formatSize(totalCleanableSize)} to free`
                : 'Scan first'
              }
            </p>
          </motion.button>
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[#8e8e93] text-xs">
                    {cat.count > 0 ? `${cat.count} items · ${formatSize(cat.size)}` : 'No items'}
                  </p>
                </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 ios-backdrop border-t border-[#38383a] safe-bottom">
        <div className="flex items-center justify-around py-2 px-4">
          ...
          <button
            onClick={() => {
              store.setActiveCategory('allPhotos')
              store.setScreen('category')
              setSelectionMode(false)
            }}
            className="flex flex-col items-center gap-0.5 py-1 ios-press"
          >
            <Grid3X3 className="w-5 h-5 text-[#8e8e93]" />
            <span className="text-[#8e8e93] text-[10px]">All Photos</span>
          </button>
          <button
            onClick={() => {
              store.setActiveCategory('duplicates')
